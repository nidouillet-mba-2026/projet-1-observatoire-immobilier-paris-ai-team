...
def generate_report(df, output_file="acheteur/data/rapport_acheteurs.csv"):
    ...
    if df['budget_max'].notna().any():
        print(f"\nBudget moyen : {df['budget_max'].mean():,.0f} €")
        print(f"Budget médian : {df['budget_max'].median():,.0f} €")
    if df['surface_min'].notna().any():
        print(f"\nSurface moyenne souhaitée : {df['surface_min'].mean():.0f} m²")
