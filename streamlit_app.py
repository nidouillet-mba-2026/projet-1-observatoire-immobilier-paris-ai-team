...
if not df.empty:
    ...
    quartiers = st.sidebar.multiselect("Quartiers", options=sorted(df['quartier'].unique()), default=df['quartier'].unique())
    date_range = st.sidebar.date_input("Période", [df['date_mutation'].min(), df['date_mutation'].max()])
    ...
    fig_trend = px.line(df_trend, x='date_mutation', y='prix_m2', labels={'prix_m2': 'Prix m² (€)', 'date_mutation': 'Date'})
    ...
    st.dataframe(df_filtered.sort_values('date_mutation', ascending=False).head(100), use_container_width=True)
else:
